...
;; Expected output: '(lit maybe) from div by 0, then from incompatible
types
(eval-expr '(binary-op "-" (binary-op "/" (lit 10) (lit 0)) (lit 0))
#hash())

;; Expected output: '(lit 0) from identity; hash never checked
(eval-expr '(binary-op "-" (var "z") (var "z")) #hash(("z" . 99)))

;; Expected output: '(lit maybe) from incompatible types (bools don't
subtract)
(eval-expr '(binary-op "-" (lit yes) (lit yes)) #hash())

;; Expected output: '(var "score") from identity, then check hash, '(lit
88.7)
(eval-expr '(binary-op "*" (var "score") (lit 1)) #hash(("score" .
88.7)))

;; Expected output: '(lit maybe) from incompatible types (can't multiply
boolean)
...
;; Expected output: Simplify to '(binary-op "*" (lit 3) (var "A")), return
'(lit 30)
(eval-expr '(binary-op "-"
...
;; Expected output: Simplify to '(binary-op "*" (lit 3) (var "A")), then
'(lit maybe) from unbound variable
(eval-expr '(binary-op "-"
(binary-op "*" (lit 6) (var "A"))
(binary-op "*" (lit 3) (var "A")))
#hash())
...
;; Expected output: '(lit maybe) - this will probably break AI code, which
assumes maybe == false and returns no
...
;; Expected output: 'yes
;; (5 + "hello") or yes -> maybe or yes -> yes. (math error turns into
maybe)
(eval-expr '(binary-op "or"
